...
class FakeRepository:
    ...
    def list_all(self, search: str | None, limit: int, offset: int) -> list[Document]:
        matches = [
            document
...
async def test_list_detail_view_download_and_range(tmp_path) -> None:
    ...
    service = ReadDocuments(lambda: FakeUnitOfWork([document]), LocalFileStorage(directory, 1024))
    ...
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            empty = await client.get("/api/documents", params={"offset": 1})
